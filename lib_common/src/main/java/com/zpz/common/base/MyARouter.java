package com.zpz.common.base;

public final class MyARouter {

    public static final String MainActivity = "/main/MainActivity";
    //设置
    public static final String SetingActivity = "/home/SetingActivity";
    //登录
    public static final String LoginActivity = "/main/LoginActivity";
    //初审
    public static final String FirstTrialActivity = "/home/FirstTrialActivity";
    //建档
    public static final String CreateProcedureActivity = "/home/CreateProcedureActivity";
    //档案管理
    public static final String FileListActivity = "/home/FileListActivity";
    //首页
    public static final String HomeFragment = "/home/HomeFragment";
    //代办
    public static final String CommissionFragment = "/home/CommissionFragment";
    //到期企业 企业年审
    public static final String ExpireCompanyActivity = "/home/ExpireCompanyActivity";
}
